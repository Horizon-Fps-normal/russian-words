...
- Android strategy: PWA (manifest + Service Worker in `public/`, registered only in production builds) is the immediate deliverable; `android/` is a Capacitor project that builds a signed release APK (`android/app/build/outputs/apk/release/app-release.apk`, copied to `release/Russian-Words-<version>.apk`). Toolchain on this machine: JDK 21 at `C:\Android\Jdk\jdk-21.0.12+8`, Android SDK at `C:\Android\Sdk` (platform 36). Build with `cd android && ./gradlew.bat assembleRelease` (JAVA_HOME/ANDROID_HOME set). Signing: `android/release-key.jks` + `keystore.properties` (password RussianWords2026). In browser/PWA environments TTS falls back to the browser build of edge-tts-universal (WebSocket Edge TTS), then to speechSynthesis. The Electron desktop app is unaffected.
